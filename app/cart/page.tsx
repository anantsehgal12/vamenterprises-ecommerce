...
interface CartItem {
  id: string;
  quantity: number;
  price?: number | null; // Added field: exact coupon-applied price stored in the cart item
  couponCode?: string | null; // Added field: optional trackable coupon code identifier
  product: {
    id: string;
    name: string;
    price: string;
...
export default function CartPage() {
  ...
  // Helper function to extract base price from item (pre-tax, coupon aware)
  const getItemBasePrice = (item: CartItem): number => {
    if (item.price) {
      // If item.price exists, it already includes tax from AddToCartForm calculation.
      // We reverse-engineer the base price to keep subtotal vs tax split accurate.
      return item.price / (1 + item.product.taxRate / 100);
    }
  ...
  const calculateSubtotal = () => {
    if (!cart?.items) return 0;
    return cart.items.reduce((total: number, item: CartItem) => {
      return total + getItemBasePrice(item) * item.quantity;
    }, 0);
  };
  ...
  const calculateTaxTotal = () => {
    ...
    return cart.items.reduce((total: number, item: CartItem) => {
      ...
      const taxAmount = basePrice * (item.product.taxRate / 100);
            ...
            <div className="lg:col-span-2">
              ...
              >
                {cart.items.map((item: CartItem) => {
                  ...
                  // Standard price with tax calculation
                  const standardFinalPrice = Math.round(
                    parseFloat(item.product.price.replace(/[^\d.]/g, "")) *
                      (1 + item.product.taxRate / 100)
                  );

                  // If custom price from applied coupon exists, use it. Otherwise, use fallback.
                  const currentDisplayPrice = item.price ? Math.round(item.price) : standardFinalPrice;
                  const hasCouponApplied = !!item.price && Math.round(item.price) < standardFinalPrice;
                            ...
                            <div className="flex gap-3 sm:flex-1 sm:items-center">
                              ...
                              <div className="flex flex-1 flex-col justify-center">
                                ...
                                <div className="flex flex-wrap gap-2 mt-1 sm:mt-2">
                                  {item.variant && (
                                    <div className="inline-flex w-fit rounded-full border border-white/10 bg-zinc-950 px-2 py-0.5 text-[10px] font-semibold text-zinc-300 sm:px-3 sm:py-1 sm:text-xs">
                                      Variant: {item.variant}
                                    </div>
                                  )}
                                  {/* Visual Coupon Badge Indicator */}
                                  {hasCouponApplied && (
                                    <div className="inline-flex w-fit rounded-full border border-[#4ca626]/20 bg-[#4ca626]/10 px-2 py-0.5 text-[10px] font-semibold text-[#9be274] sm:px-3 sm:py-1 sm:text-xs">
                                      Coupon Saved: {item.couponCode || "Applied"}
                                    </div>
                                  )}
                                </div>

                                <div className="mt-1.5 flex items-center gap-2 sm:mt-4 sm:gap-3">
                                  <span className={`text-base font-black sm:text-2xl ${hasCouponApplied ? "text-[#8ef065]" : "text-white"}`}>
                                    ₹{currentDisplayPrice}
                                  </span>
                                  {(hasCouponApplied || item.product.mrp) && (
                            ...
                            <div className="flex items-center justify-between border-t border-white/5 pt-3 sm:border-t-0 sm:pt-0">
                              ...
                              <div className="flex items-center gap-4 sm:flex-col sm:items-end sm:gap-2">
                                <div className="text-right">
                                  <p className="hidden text-sm text-zinc-400 sm:block">Total</p>
