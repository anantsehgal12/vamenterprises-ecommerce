...
export default function ProductsPage() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (errorData.error === "Cannot set product to live when stock is 0") {
          ...
          if (product) {
            toast.error(
              `Due to zero stock, the product "${product.name}" cannot be set to live.`,
              {
                style: {
                  borderRadius: "10px",
                  background: "#333",
          ...
          throw new Error(
            errorData.error || "Failed to update product live status",
          );
        }
        return;
      }
  ...
  const handleToggleOptional = async (productId: string, checked: boolean) => {
    try {
      const newIsOptional = checked ? "Yes" : "No";
      const response = await fetch(`/api/products/${productId}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ isOptional: newIsOptional }),
      });

      if (!response.ok) {
        throw new Error("Failed to update product optional status");
      }
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {filteredProducts.map((product) => {
                          ...
                          const taxAmount =
                            price * (parseFloat(product.taxRate) / 100);
                          ...
                          return (
                            <TableRow key={product.id}>
                              ...
                              <TableCell className="text-[#7ddc56] font-bold">
                                ₹{Math.round(price)}
                              </TableCell>
                              <TableCell className="text-[#7ddc56] font-bold">
                              ...
                              <TableCell>
                                <Switch
                                  ...
                                  onCheckedChange={(checked) =>
                                    handleToggleLive(product.id, checked)
                                  }
                              ...
                              <TableCell>
                                <Switch
                                  checked={product.isOptional === "Yes"}
                                  onCheckedChange={(checked) =>
                                    handleToggleOptional(product.id, checked)
                                  }
                                  aria-label={`Toggle ${product.name} optional status`}
                                />
                              </TableCell>
                              <TableCell>
                                <div className="flex space-x-2">
                                  ...
                                  <AlertDialog>
                                    ...
                                    <AlertDialogContent>
                                      ...
                                      <AlertDialogFooter>
                                        <AlertDialogCancel className="border-white/10 bg-[#181818] hover:bg-[#1f1f1f]">
                                          Cancel
                                        </AlertDialogCancel>
                                        <AlertDialogAction
                                          onClick={() =>
                              ...
                              <AlertDialog>
                                <AlertDialogTrigger asChild>
                                  <Button
                                    variant="outline"
                                    size="sm"
                                    className="border-red-500/20 bg-red-500/10 hover:bg-red-500/20 text-red-400 flex-1"
                                  >
                                    <Trash2 className="h-4 w-4 mr-1" />
                                    Delete
                                  </Button>
